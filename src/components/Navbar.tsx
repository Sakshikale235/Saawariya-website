...
export function Navbar() {
  const { cartCount, wishlistCount, searchQuery, setSearchQuery, setCategoryFilter } = useApp();
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (localQuery !== searchQuery) {
        ...
        if (localQuery.trim() !== '' && !isShopPage) {
          navigate('/shop');
        }
      }
    }, 300);
    return () => clearTimeout(timer);
  }, [localQuery, searchQuery, setSearchQuery, isShopPage, navigate]);
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <div className="w-10 h-10 rounded-full border-2 border-[#C4A35A] flex items-center justify-center bg-[#F7F2E8]">
                <span
                  className="font-serif text-[#6B1D1D] text-lg font-bold"
              ...
              <div className="hidden sm:block">
                <h1
                  className="text-[#6B1D1D] text-lg font-bold tracking-wide leading-tight"
                  style={{ fontFamily: "'Cinzel', serif" }}
                >
                  SAAWARIYA
                </h1>

                <p className="text-[#6B6560] text-[10px] tracking-[0.2em] uppercase">
                  Indian Coded
                </p>
              </div>
            </button>
            ...
            <div className="hidden md:flex items-center gap-2 bg-[#F7F2E8] border border-transparent focus-within:border-[#C4A35A]/50 rounded-lg px-3 py-1.5 w-60 lg:w-72 transition-all duration-300">
              ...
              {localQuery && (
                <button
                  ...
                  className="p-0.5 hover:bg-gray-200/50 rounded-full"
                >
            ...
            <div ref={searchRef} className="relative md:hidden">
              ...
              {searchOpen && (
                <div className="absolute right-0 top-full mt-2 w-72 sm:w-96 bg-white rounded-xl shadow-lg border border-gray-100 p-3 z-50">
                  ...
                  <div className="mt-2">
                    <p className="text-xs text-[#6B6560] uppercase tracking-wider mb-2 px-1">
                      Popular Searches
                    </p>
              ...
              <div className="px-4 py-3 space-y-1">
                ...
                {navItems.map((item) => (
                  <button
                    ...
                    className="block w-full text-left px-3 py-2.5 text-sm text-[#2C2C2C] hover:bg-[#F7F2E8] rounded-lg"
                  >
                    {item.label}
                  </button>
                ))}
                ...
                <button
                  ...
                  className="block w-full text-left px-3 py-2.5 text-sm hover:bg-[#F7F2E8] rounded-lg"
                >
                  My Profile
                </button>
