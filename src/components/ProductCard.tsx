...
export function ProductCard({ product }: { product: Product }) {
  const { navigate, addToCart, addToWishlist, removeFromWishlist, isInWishlist } = useApp();
    ...
    >
      <div className="relative aspect-[3/4] overflow-hidden bg-[#F7F2E8]">
        ...
        >
          <Heart
            size={16}
            className={inWishlist ? 'fill-[#6B1D1D] text-[#6B1D1D]' : 'text-[#6B6560]'}
          />
        </button>
        <div
          className={`absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/40 to-transparent transition-opacity duration-300 ${hovered ? 'opacity-100' : 'opacity-0'}`}
        >
          <button
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-2 mb-2">
          <div className="flex items-center gap-1">
            <Star size={12} className="fill-[#C4A35A] text-[#C4A35A]" />
            <span className="text-xs font-medium text-[#2C2C2C]">{product.rating}</span>
          </div>
